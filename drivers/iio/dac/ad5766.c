...
static int ad5766_get_ch_dither_source(struct iio_dev *dev,
				       const struct iio_chan_spec *chan);
static int ad5766_set_ch_dither_source(struct iio_dev *dev,
				       const struct iio_chan_spec *chan,
				       unsigned int mode);

static int ad5766_get_ch_dither_scale(struct iio_dev *dev,
				      const struct iio_chan_spec *chan);
static int ad5766_set_ch_dither_scale(struct iio_dev *dev,
				      const struct iio_chan_spec *chan,
				      unsigned int mode);
...
static const struct iio_enum ad5766_dither_source_enum = {
	...
	.set = ad5766_set_ch_dither_source,
	.get = ad5766_get_ch_dither_source,
};

static const struct iio_enum ad5766_dither_scale_enum = {
	.items = ad5766_dither_scales,
	.num_items = ARRAY_SIZE(ad5766_dither_scales),
	.set = ad5766_set_ch_dither_scale,
	.get = ad5766_get_ch_dither_scale,
};
 ...
 *			2 config bits per channel: 0 - No Dither, 1 - N0, 2 - N1
 * @dither_scale:	Selects between 4 possible sources:
 *			No scale, 75%, 50%, 25%
...
static int ad5766_set_dither_source(struct ad5766_state *st,
				    unsigned int mode);
static int ad5766_set_dither_scale(struct ad5766_state *st,
				   unsigned int scale);
	...
	enum ad5766_voltage_range range)
{
	...
	/* Set dither to normal operation for all 16 channels */
	...
	/* No scale for all 16 channels */
	st->dither_scale = 0;
	ret = ad5766_set_dither_scale(st, st->dither_scale);
	if (ret)
		return ret;
	...
	/* Set No Dither for all 16 channels */
	st->dither_source = 0;
	return ad5766_set_dither_source(st, st->dither_source);
}
...
static int ad5766_get_ch_dither_source(struct iio_dev *dev, const struct iio_chan_spec *chan)
{
			  ...
			  unsigned int mode)
{
	...
	return _ad5766_spi_write(st, AD5766_CMD_DITHER_SIG_2,
			  ...
			  unsigned int mode)
{
	...
	bitmap_clear((unsigned long *)&st->dither_source, chan->channel * 2, 2);
				   ...
				   unsigned int scale)
{
	...
	return _ad5766_spi_write(st, AD5766_CMD_DITHER_SCALE_2,
				      ...
				      unsigned int scale)
{
	...
	bitmap_clear((unsigned long *)&st->dither_scale, chan->channel * 2, 2);
	st->dither_scale |= scale << (chan->channel * 2);

	return ad5766_set_dither_scale(st, st->dither_scale);
}
